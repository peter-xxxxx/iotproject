...
class sensorObj(pyupm_mpu9150.MPU60X0):
    ...
    def __init__(self, frequency = 200, # Hz
        ...
        self.const.DELTA_T    = 1.0 / frequency
    ...
    UpdateData
    ================================================================================
    update the data in the sensor, and calculate the Quaternion
    ================================================================================
    """
    def UpdateData(self):
        self.RecieveData()
        delta_theta = self.Gyr * self.const.DELTA_T
        theta = np.sqrt(np.dot(delta_theta, delta_theta.T))
        theta_matrix = np.array([[0, -delta_theta[0], -delta_theta[1], -delta_theta[2]],
                                 [delta_theta[0], 0, delta_theta[2], -delta_theta[1]],
                                 [delta_theta[1], -delta_theta[2], 0, delta_theta[0]],
                                 [delta_theta[2], delta_theta[1], -delta_theta[0], 0]])
        self.Quat = np.matmul(math.cos(theta / 2) * np.identity(4) +
                                math.sin(theta / 2) * theta_matrix / theta, self.Quat)
        Quat = self.Quat
        #self.Euler[0] = math.atan2(2*(Quat[0]*Quat[1]+Quat[2]*Quat[3]),(1-2*(Quat[1]*Quat[1]+Quat[2]*Quat[2])))
        #self.Euler[1] = math.asin(2*(Quat[0]*Quat[2]+Quat[1]*Quat[3]))
        #self.Euler[2] = math.atan2(2*(Quat[0]*Quat[3]+Quat[1]*Quat[2]),(1-2*(Quat[2]*Quat[2]+Quat[3]*Quat[3])))
        self.Euler[0] += self.Gyr[1] * self.const.DELTA_T * 2
        self.Euler[1] += self.Gyr[0] * self.const.DELTA_T * 2
        self.Euler[2] += self.Gyr[2] * self.const.DELTA_T * 2
        #print(self.Euler)
